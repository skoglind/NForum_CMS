...
Select Case sAction
  Case "save" ' Spara
    ...
    Con_Open
      ...
      SQL = "SELECT * FROM cms_Konsoltrix WHERE xID = " & CLng(lID) & sbFilter
      rsDB.Open SQL, Con, 1, 3
    
      ' #### FELHANTERING ####
        bErr = False
      
        If lSpelID = 0 Then bErr = True : nMessage = "<p>Inget har lagrats i databasen då du valt inte har valt en konsol.</p>"
      ...
      ' ######################
        ...
        Else
          ...
          Select Case lStatus
            Case 2  ' Inväntar publicering
              nStatus = 2
              Call AddLogg("KONSOLTRIX","PUBLICERING [INVÄNTA]",lID)
            Case 3  ' Publicering nekad
              nStatus = 3
              Call AddLogg("KONSOLTRIX","PUBLICERING [NEKAD]",lID)
            Case 4  ' Publicerad
              nStatus = 4
              ...
              Select Case lPublTyp
                Case 1  ' DIREKT
                  ...
                  Call AddLogg("KONSOLTRIX","PUBLICERING [GODKÄNN DIREKT]",lID)
                Case 2  ' IMORGON
                  rsDB("xDatumPublicerad") = CDate(DateAdd("d", 1, Date) & " " & PUBL_TID)
                  Call AddLogg("KONSOLTRIX","PUBLICERING [GODKÄNN IMORGON]",lID)
                Case 3  ' NÄSTA VARDAG
                  Do Until IsVardag
                  ...
                  Call AddLogg("KONSOLTRIX","PUBLICERING [GODKÄNN VARDAG]",lID)
                Case 4  ' VALT DATUM
                  If Not lPublDatum > #2025-01-01# Then 
                    rsDB("xDatumPublicerad") = CDate(lPublDatum & " " & PUBL_TID)
                    Call AddLogg("KONSOLTRIX","PUBLICERING [GODKÄNN DATUM]",lID)
                  Else
                    bDontChange = True
                  End If
                Case Else
        ...
        lID = rsDB("xID")
        If bIsNew Then AddLogg "KONSOLTRIX","SKAPA",lID
  ...
  Case "del" ' Radera
    Con_Open
      ...
      Set rsDB = Server.CreateObject("ADODB.RecordSet")
        ...
        For Each oID IN allID
          ...
          SQL = "SELECT * FROM cms_Konsoltrix WHERE xID = " & CLng(oID)
          rsDB.Open SQL, Con, 1, 3
        
          If Not rsDB.Eof Then
            If GetAcc("CMS111") Then
              Call AddLogg("KONSOLTRIX","RADERA [TOTAL]",rsDB("xID"))
    ...
    Session.value("PBM_Lank")       = "modul/Texter/KonsolTrix/_show.asp?" & sRebuild
