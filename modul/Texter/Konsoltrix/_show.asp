<% 
  cON_PAGE = "Alla tips & trix - Tips & Trix (Konsol) - CMS"
%>
...
<!--#INCLUDE FILE="../../../_deftop.asp"-->
  ...
  SQL = "SELECT * FROM cms_Konsoltrix LEFT JOIN fsBB_Anv ON fsBB_Anv.aID = cms_Konsoltrix.xSkapadAv WHERE " & sFilterText & " ORDER BY xStatus ASC, xDatumPublicerad DESC, xDatumSkapad DESC"
  rsDB.Open SQL, Con
  
  ' #### PAGING ####
  lMaxPosterPerSida = MAXPERPAGE
  lAntalPoster = Con.ExeCute("SELECT COUNT(*) FROM cms_Konsoltrix LEFT JOIN fsBB_Anv ON fsBB_Anv.aID = cms_Konsoltrix.xSkapadAv WHERE " & sFilterText)(0)
  ...
  <div class="datablock rect">
    <div class="legend">Alla tips & trix (Konsol)</div>
    <div class="editbar">
      ...
      <ul class="list">
        <% For zx = 1 To lMaxPosterPerSida %>
          <% If rsDB.EOF Then Exit For %>
            ...
            <li <% If IsEven Then Response.Write(" class='IO'") %> style="background-image: url('/design/icons/papper_<% = rsDB("xStatus") %>.png')"> <span class="category"><% = rsDB("xKonsolID") %></span> <input type="checkbox" name="chk_id<% = rsDB("xID") %>" value="YES"> <a href="_edit.asp?e=<% = rsDB("xID") %>&<% = sRebuild %>"><% = sEncode(rsDB("xTitel")) %></a> <span class="status">Av: <% = rsDB("aAnvNamn") %></span> </li>
          <% rsDB.MoveNext %>
        <% Next %>
      </ul>
  ...
  <div class="databox info">
    ...
    <div class="inner">
      ...
      SQL = "SELECT *, " & _
            "(SELECT COUNT(*) FROM cms_Konsoltrix WHERE xStatus = 2 " & sBFilter & ") AS iAntalStatus2, " & _
            "(SELECT COUNT(*) FROM cms_Konsoltrix WHERE xStatus = 3 " & sBFilter & ") AS iAntalStatus3, " & _
            "(SELECT COUNT(*) FROM cms_Konsoltrix WHERE xStatus = 4 " & sBFilter & ") AS iAntalStatus4 " & _
            "FROM cms_Konsoltrix"
