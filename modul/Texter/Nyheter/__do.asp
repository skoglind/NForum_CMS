...
Select Case sAction
  Case "save" ' Spara
    ...
    Con_Open
      ...
      ' #### FELHANTERING ####
        ...
        If Len(sTitel) < 1 Then bErr = True : nMessage = "<p>Inget har lagrats i databasen då du inte har angett någon titel för nyheten.</p>"
        
        If Not rsDB.EOF Then
          bIsNew = False
          If Not GetAcc("CMS111") And CLng(rsDB("nSkapadAv")) <> CLng(cCMS_ID) Then bErr = True : nMessage = "<p>Inget har lagrats i databasen då du saknar behörighet att ändra denna nyhet.</p>"
          If Not GetAcc("CMS11") And (CLng(rsDB("nStatus")) = CLng(4) Or CLng(rsDB("nStatus")) = CLng(2)) Then bErr = True : nMessage = "<p>Inget har lagrats i databasen då du saknar behörighet att ändra denna nyhet.</p>"
        Else
          bIsNew = True
        End If
          ...
          If lStatus <> 0 And GetAcc("CMS11") Then
            Select Case lStatus
              Case 1  ' Under bearbetning
                nStatus = 1
                Call AddLogg("NYHET","PUBLICERING [ÅTER BEARBETNING]",lID)
              Case 2  ' Inväntar publicering
                nStatus = 2
                Call AddLogg("NYHET","PUBLICERING [INVÄNTA]",lID)
              ...
              Case 4  ' Publicerad
                ...
                Select Case lPublTyp
                  Case 1  ' DIREKT
                    ...
                    Call AddLogg("NYHET","PUBLICERING [GODKÄNN DIREKT]",lID)
                  Case 2  ' IMORGON
                    rsDB("nDatumPublicerad") = CDate(DateAdd("d", 1, Date) & " " & PUBL_TID)
                    Call AddLogg("NYHET","PUBLICERING [GODKÄNN IMORGON]",lID)
                  Case 3  ' NÄSTA VARDAG
                    Do Until IsVardag
                    ...
                    Call AddLogg("NYHET","PUBLICERING [GODKÄNN VARDAG]",lID)
                  Case 4  ' VALT DATUM
                    If Not lPublDatum > #2025-01-01# Then 
                      rsDB("nDatumPublicerad") = CDate(lPublDatum & " " & PUBL_TID)
                      Call AddLogg("NYHET","PUBLICERING [GODKÄNN DATUM]",lID)
                    Else
                      bDontChange = True
                    End If
                  Case Else
  ...
  Case "await" ' Invänta publicering
          ...
          For Each oID IN allID
            ...
            If Not rsDB.Eof Then
              If rsDB("nStatus") = 1 Then
                rsDB("nStatus") = 2
                rsDB.Update
                Call AddLogg("NYHET","PUBLICERING [INVÄNTA]",rsDB("nID"))
              End If
            End If
    ...
    Session.value("PBM_Message")    = "<h2>Information: Statusändring slutförd</h2><p>De markerade nyheter som du hade behörighet att ändra status på har nu ändrats till ""Inväntar publicering"".</p><p>Klicka på ""fortsätt"" för att gå vidare...</p>"
  ...
  Case "unawait" ' Åter under bearbetning
          ...
          For Each oID IN allID
            ...
            If Not rsDB.Eof Then
              If rsDB("nStatus") = 2 Then
                rsDB("nStatus") = 1
                rsDB.Update
                Call AddLogg("NYHET","PUBLICERING [ÅTER BEARBETNING]",rsDB("nID"))
              End If
            End If
    ...
    Session.value("PBM_Message")    = "<h2>Information: Statusändring slutförd</h2><p>De markerade nyheter som du hade behörighet att ändra status på har nu ändrats till ""Under bearbetning"".</p><p>Klicka på ""fortsätt"" för att gå vidare...</p>"
    Session.value("PBM_Lank")       = "modul/Texter/Nyheter/_show.asp?" & sRebuild
  ...
  Case "del" ' Radera
    ...
    Session.value("PBM_Message")    = "<h2>Information: Radering slutförd</h2><p>De markerade nyheter som du hade behörighet att radera är nu borta.</p><p>Klicka på ""fortsätt"" för att gå vidare...</p>"
    Session.value("PBM_Lank")       = "modul/Texter/Nyheter/_show.asp?" & sRebuild
