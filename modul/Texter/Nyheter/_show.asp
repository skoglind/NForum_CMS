<% 
  cON_PAGE = "Alla nyheter - Nyheter - CMS"
%>
...
<!--#INCLUDE FILE="../../../_deftop.asp"-->
  ...
  Select Case sFilter
    Case "own"    : sFilter = "own"   : sFilterText = "NOT nStatus = 0 AND nSkapadAv = " & cCMS_ID
    Case "del"    : sFilter = "del"   : If GetAcc("CMS111") Then sFilterText = "nStatus = 0" Else sFilterText = "NOT nStatus = 0"
    Case "publ"   : sFilter = "publ"  : sFilterText = "nStatus = 4"
    Case "deny"   : sFilter = "deny"  : sFilterText = "nStatus = 3"
    Case "await"  : sFilter = "await" : sFilterText = "nStatus = 2"
    Case "const"  : sFilter = "const" : sFilterText = "nStatus = 1"
    Case Else     : sFilter = ""      : sFilterText = "NOT nStatus = 0"
  End Select
  ' ################
  ...
  SQL = "SELECT * FROM cms_Nyheter LEFT JOIN fsBB_Anv ON fsBB_Anv.aID = cms_Nyheter.nSkapadAv WHERE " & sFilterText & sBFilter & " ORDER BY nStatus ASC, nDatumPublicerad DESC, nDatumSkapad DESC"
  rsDB.Open SQL, Con
  
  ' #### PAGING ####
  lMaxPosterPerSida = MAXPERPAGE
  lAntalPoster = Con.ExeCute("SELECT COUNT(*) FROM cms_Nyheter LEFT JOIN fsBB_Anv ON fsBB_Anv.aID = cms_Nyheter.nSkapadAv WHERE " & sFilterText & sBFilter)(0)
  ...
  <div class="datablock rect">
    <div class="legend">Alla nyheter</div>
    <div class="editbar">
      ...
      <ul class="list">
        <% For zx = 1 To lMaxPosterPerSida %>
          <% If rsDB.EOF Then Exit For %>
            ...
            <li <% If IsEven Then Response.Write(" class='IO'") %> style="background-image: url('/design/icons/papper_<% = rsDB("nStatus") %>.png')"> <span class="category"><% = lstKategori(rsDB("nKategori")) %></span> <input type="checkbox" name="chk_id<% = rsDB("nID") %>" value="YES"> <a href="_edit.asp?e=<% = rsDB("nID") %>&<% = sRebuild %>"><% If rsDB("nDatumPublicerad") > Now AND rsDB("nStatus") = 4 Then %> <img src="/design/icons/ko.png" border=0 alt="KÖ" title="Köar, kommer publiceras enligt kö"> <% End If %><% = sEncode(rsDB("nTitel")) %></a> <span class="status">Av: <% = rsDB("aNamn") %></span> </li>
          <% rsDB.MoveNext %>
        <% Next %>
      </ul>
  ...
  <div class="databox info">
    ...
    <div class="inner">
      ...
      SQL = "SELECT *, " & _
            "(SELECT COUNT(*) FROM cms_Nyheter WHERE nStatus = 0 " & sBFilter & ") AS iAntalStatus0, " & _
            "(SELECT COUNT(*) FROM cms_Nyheter WHERE nStatus = 1 " & sBFilter & ") AS iAntalStatus1, " & _
            "(SELECT COUNT(*) FROM cms_Nyheter WHERE nStatus = 2 " & sBFilter & ") AS iAntalStatus2, " & _
            "(SELECT COUNT(*) FROM cms_Nyheter WHERE nStatus = 3 " & sBFilter & ") AS iAntalStatus3, " & _
            "(SELECT COUNT(*) FROM cms_Nyheter WHERE nStatus = 4 " & sBFilter & ") AS iAntalStatus4 " & _
            "FROM cms_Nyheter"
