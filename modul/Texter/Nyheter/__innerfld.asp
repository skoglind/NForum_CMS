...
SQL = "SELECT *, anvDB1.aNamn AS Anv1, anvDB2.aNamn AS Anv2 " & _
      ...
      "WHERE nID = " & CLng(cADD_ID) & sBFilter
rsDB.Open SQL, Con
%>

<% If rsDB.EOF Then %>
  <div class="minimess">Nyheten &auml;r &auml;nnu inte sparad.</div>
<% Else %>
  <% lStatus = rsDB("nStatus") %>
  ...
  <div class="field"><strong>Skapad:</strong><br><p><% = rsDB("nDatumSkapad") %></p></div>
  ...
  <% If lStatus = 4 Then %>
    <div class="innerseparator"> </div>
    <% If rsDB("nDatumPublicerad") > Now Then %>
      <div class="field"><strong>Kommer publiceras:</strong><br>
    <% Else %>
      <div class="field"><strong>Publicerad:</strong><br>
    <% End If %>
    <p><% = FormatDateTime(rsDB("nDatumPublicerad"), vbShortDate) %> (<% = Server.HTMLEncode(WeekDayName(weekday(rsDB("nDatumPublicerad")))) %>)</p></div>
...
<div class="innerseparator"> </div>
<div class="field"><strong>Flash:</strong> 180x120px<br>

<%
Con_Open
 Set rsTN = Server.CreateObject("ADODB.RecordSet")
    SQL = "SELECT * FROM cms_Nyheter LEFT JOIN cms_Bild ON cms_Nyheter.nFlash = cms_Bild.bID WHERE nID = " & CLng(cADD_ID)
    rsTN.Open SQL, Con
...
<form id="smallupload" method="POST" enctype="multipart/form-data">
  ...
  <input type="hidden" value="nyheter" name="sArea">
</form>
<% End If %>
...
<% If anyHit Then %>
  ...
  <% If GetAcc("CMS111") Then %><div class="field" style="text-align: center;"><input type="button" value="Radera bild" onclick="if(confirm('Vill du radera bilden?')){smalldeleteimg(<% = cADD_ID %>,'nyheter');}"></div><% End If %>
<% End If %>