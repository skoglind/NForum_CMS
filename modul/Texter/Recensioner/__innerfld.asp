...
SQL = "SELECT *, anvDB1.aNamn AS Anv1, anvDB2.aNamn AS Anv2 " & _
      ...
      "WHERE rID = " & CLng(cADD_ID) & sBFilter
rsDB.Open SQL, Con
%>

<% If rsDB.EOF Then %>
  <div class="minimess">Recensionen &auml;r &auml;nnu inte sparad.</div>
<% Else %>
  <% lStatus = rsDB("rStatus") %>
  ...
  <div class="field"><strong>Skapad:</strong><br><p><% = rsDB("rDatumSkapad") %></p></div>
  ...
  <% If lStatus = 4 Then %>
    <div class="innerseparator"> </div>
    <% If rsDB("rDatumPublicerad") > Now Then %>
      <div class="field"><strong>Kommer publiceras:</strong><br>
    <% Else %>
      <div class="field"><strong>Publicerad:</strong><br>
    <% End If %>
    <p><% = FormatDateTime(rsDB("rDatumPublicerad"), vbShortDate) %> (<% = Server.HTMLEncode(WeekDayName(weekday(rsDB("rDatumPublicerad")))) %>)</p></div>
...
Con_Open
 Set rsTN = Server.CreateObject("ADODB.RecordSet")
    SQL = "SELECT * FROM cms_Recensioner LEFT JOIN cms_Bild ON cms_Recensioner.rFlash = cms_Bild.bID WHERE rID = " & CLng(cADD_ID)
    rsTN.Open SQL, Con
...
<form id="smallupload" method="POST" enctype="multipart/form-data">
  ...
  <input type="hidden" value="recensioner" name="sArea">
</form>
<% End If %>
...
<% If anyHit Then %>
  ...
  <% If GetAcc("CMS111") Then %><div class="field" style="text-align: center;"><input type="button" value="Radera bild" onclick="if(confirm('Vill du radera bilden?')){smalldeleteimg(<% = cADD_ID %>,'recensioner');}"></div><% End If %>
<% End If %>