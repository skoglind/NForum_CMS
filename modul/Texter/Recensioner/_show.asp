<% 
  cON_PAGE = "Alla recensioner - Recensioner - CMS"
%>
...
<!--#INCLUDE FILE="../../../_deftop.asp"-->
  ...
  Select Case sFilter
    Case "own"    : sFilter = "own"   : sFilterText = "NOT rStatus = 0 AND rSkapadAv = " & cCMS_ID
    Case "del"    : sFilter = "del"   : If GetAcc("CMS111") Then sFilterText = "rStatus = 0" Else sFilterText = "NOT rStatus = 0"
    Case "publ"   : sFilter = "publ"  : sFilterText = "rStatus = 4"
    Case "deny"   : sFilter = "deny"  : sFilterText = "rStatus = 3"
    Case "await"  : sFilter = "await" : sFilterText = "rStatus = 2"
    Case "const"  : sFilter = "const" : sFilterText = "rStatus = 1"
    Case Else     : sFilter = ""      : sFilterText = "NOT rStatus = 0"
  End Select
  ' ################
  ...
  SQL = "SELECT * FROM cms_Recensioner LEFT JOIN fsBB_Anv ON fsBB_Anv.aID = cms_Recensioner.rSkapadAv WHERE " & sFilterText & sBFilter & " ORDER BY rStatus ASC, rDatumPublicerad DESC, rDatumSkapad DESC"
  rsDB.Open SQL, Con
  
  ' #### PAGING ####
  lMaxPosterPerSida = MAXPERPAGE
  lAntalPoster = Con.ExeCute("SELECT COUNT(*) FROM cms_Recensioner LEFT JOIN fsBB_Anv ON fsBB_Anv.aID = cms_Recensioner.rSkapadAv WHERE " & sFilterText & sBFilter)(0)
  ...
  <div class="datablock rect">
    <div class="legend">Alla recensioner</div>
    <div class="editbar">
      ...
      <ul class="list">
        <% For zx = 1 To lMaxPosterPerSida %>
          <% If rsDB.EOF Then Exit For %>
            ...
            <li <% If IsEven Then Response.Write(" class='IO'") %> style="background-image: url('/design/icons/papper_<% = rsDB("rStatus") %>.png')"> <span class="category"><% = lstKonsol(rsDB("rKategori")) %></span> <input type="checkbox" name="chk_id<% = rsDB("rID") %>" value="YES"> <a href="_edit.asp?e=<% = rsDB("rID") %>&<% = sRebuild %>"><% If rsDB("rDatumPublicerad") > Now Then %> <img src="/design/icons/ko.png" border=0 alt="KÖ" title="Köar, kommer publiceras enligt kö"> <% End If %><% = sEncode(rsDB("rTitel")) %></a> <span class="status">Av: <% = rsDB("aNamn") %></span> </li>
          <% rsDB.MoveNext %>
        <% Next %>
      </ul>
  ...
  <div class="databox info">
    ...
    <div class="inner">
      ...
      SQL = "SELECT *, " & _
            "(SELECT COUNT(*) FROM cms_Recensioner WHERE rStatus = 0 " & sBFilter & ") AS iAntalStatus0, " & _
            "(SELECT COUNT(*) FROM cms_Recensioner WHERE rStatus = 1 " & sBFilter & ") AS iAntalStatus1, " & _
            "(SELECT COUNT(*) FROM cms_Recensioner WHERE rStatus = 2 " & sBFilter & ") AS iAntalStatus2, " & _
            "(SELECT COUNT(*) FROM cms_Recensioner WHERE rStatus = 3 " & sBFilter & ") AS iAntalStatus3, " & _
            "(SELECT COUNT(*) FROM cms_Recensioner WHERE rStatus = 4 " & sBFilter & ") AS iAntalStatus4 " & _
