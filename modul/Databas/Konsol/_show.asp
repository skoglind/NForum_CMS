<% 
  cON_PAGE = "Alla konsoler - Konsoler - CMS"
%>
...
<!--#INCLUDE FILE="../../../_deftop.asp"-->
  ...
  If CStr(lstKonsol(sFilter)) <> CStr(lstKonsol(0)) Then
    sAddFilter = " AND kKonsol = " & CLng(sFilter)
  Else
    sAddFilter = ""
  End If
  
  If Len(sQ) > 2 Then
    sAddFilter2 = " AND (tTitel LIKE '%" & sQ & "%' OR kNyckelord = '%" & sQ & "%' OR kID IN(SELECT tKonsolID From cms_KonsolTitlar WHERE tTitel LIKE '%" & sQ & "%'))"
  Else
    sAddFilter2 = ""
  End If
  ...
  SQL = "SELECT *, " & _
        "(SELECT COUNT(*) FROM cms_Bild LEFT JOIN cms_KonsolTitlar AS kTit ON cms_Bild.bID IN(kTit.tBoxart_BoxFram,kTit.tBoxart_BoxBak,kTit.tBoxart_Konsol,kTit.tBoxart_Manual) WHERE tKonsolID = cms_Konsol.kID) AS aAntalBilder " & _
        "FROM cms_Konsol LEFT JOIN cms_Konsoltitlar ON cms_Konsol.kStandard_Titel = cms_Konsoltitlar.tID WHERE 1=1 " & sAlfaFilter & sAddFilter & sAddFilter2 & " ORDER BY tTitel ASC"
  rsDB.Open SQL, Con
  
  ' #### PAGING ####
  lMaxPosterPerSida = MAXPERPAGE
  lAntalPoster = Con.ExeCute("SELECT COUNT(*) FROM cms_Konsol LEFT JOIN cms_Konsoltitlar ON cms_Konsol.kStandard_Titel = cms_Konsoltitlar.tID WHERE 1=1 " & sAlfaFilter & sAddFilter & sAddFilter2)(0)
  ...
  <div class="datablock rect">
    <div class="legend">Alla konsoler</div>
    <div class="editbar">
      ...
      <ul class="list">
        <% For zx = 1 To lMaxPosterPerSida %>
          <% If rsDB.EOF Then Exit For %>
            ...
            <li <% If IsEven Then Response.Write(" class='IO'") %> style="background-image: url('/design/icons/console<% If Not rsDB("kSynlig") Then Response.Write("_hid") %>.png')"> <% If rsDB("aAntalBilder") > 0 Then %><span class="number"><strong><% = rsDB("aAntalBilder") %></strong><br>bild(er)</span><% End If %> <input type="checkbox" name="chk_id<% = rsDB("kID") %>" value="YES"> <a href="_edit.asp?e=<% = rsDB("kID") %>&<% = sRebuild %>"><% = sEncode(rsDB("tTitel")) %></a> <span class="status"><% = lstKonsol(rsDB("kKonsol")) %></span> </li>
          <% rsDB.MoveNext %>
        <% Next %>
      </ul>
  ...
  <div class="databox info">
    ...
    <div class="inner">
      ...
      For zx = 1 To lstKonsolShort(0)
        sListSQL = sListSQL & "(SELECT COUNT(*) FROM cms_Konsol WHERE kKonsol = " & CLng(zx) & ") AS iAntalKonsol" & CLng(zx) & ", " 
      Next
      
      Set rsStats = Server.CreateObject("ADODB.RecordSet")
      SQL = "SELECT *, " & sListSQL & _
            "(SELECT COUNT(*) FROM cms_Konsol WHERE kSynlig = 1) AS iAntalStatus0, " & _
            "(SELECT COUNT(*) FROM cms_Konsol WHERE kSynlig = 0) AS iAntalStatus1, " & _
            "(SELECT COUNT(*) FROM cms_Konsoltitlar) AS aAntalTitlar " & _
            "FROM cms_Konsol"
      ...
      <table class="list" cellpadding=0 cellspacing=0 style="float: left;">
        <tr><td class="td1"> <img src="/design/icons/console_sm.png"> </td><td clasS="td2"> <% = lStatus_0 %> </td><td class="td3"> synliga konsoler </td></tr>
        <tr><td class="td1"> <img src="/design/icons/console_hid_sm.png"> </td><td clasS="td2"> <% = lStatus_1 %> </td><td class="td3"> dolda konsoler </td></tr>
      </table>
      <div class="innerseparator"> </div>
      <table class="list" cellpadding=0 cellspacing=0 style="float: left;">
        <tr><td class="td1"> <img src="/design/icons/papper_1_sm.png"> </td><td clasS="td2"> <% = lAntalTitlar %> </td><td class="td3"> konsoltitlar </td></tr>
      </table>
      <div class="innerseparator"> </div>
      <table class="list" cellpadding=0 cellspacing=0 style="float: left;">
        <% For zx = 1 To lstKonsolShort(0) %>
          <tr><td class="td1"> <img src="/design/icons/console_sm.png"> </td><td class="td2"> <% = lCountKonsol(zx) %> </td><td class="td3"> <a href="?f=<% = zx %>&alfa=<% = sSendAlfa %>"><% = lstKonsolShort(zx) %></a> </td></tr>
        <% Next %>
      </table>
    </div>
  </div>
