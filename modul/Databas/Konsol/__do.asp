...
Select Case sAction
  Case "save" ' Spara
    ...
    Con_Open
      ...
      ' ######################
        ...
        rsDB("kKonsol")       = CLng(sKonsol)
        rsDB("kNyckelord")    = sNyckelord
        rsDB("kTextM")        = sTextM
        
        rsDB("kSynlig")       = sSynlig
        
        rsDB("kDatumSparad") = Now
        saveDate = "Sparad (" & FormatDateTime(Now, vbShortDate) & " " & FormatDateTime(Now, vbShortTime) & ")"
        
        rsDB.Update
        
        lID = rsDB("kID")
        If bIsNew Then AddLogg "KONSOL","SKAPA",lID
        
        ' #### TITLAR ####
          Set rsT = Server.CreateObject("ADODB.RecordSet")
          SQL = "SELECT * FROM cms_Konsoltitlar WHERE tKonsolID = " & CLng(lID) & " ORDER BY tID ASC"
          ...
          For zx = 0 To UBound(nID)
            If Not nAction(zx) Then
              SQL = "SELECT * FROM cms_Konsoltitlar WHERE tSparadNyckel = '" & MakeLegal(nID(zx)) & "' AND tSparadAv = " & CLng(cCMS_ID)
              rsT.Open SQL, Con
          ...
          Con.ExeCute("UPDATE cms_Konsol SET kStandard_Titel = " & CLng(nUseTitel) & " WHERE kID = " & CLng(lID))
        ' ################
        
        ' #### CLEAN-UP ####
          Con.ExeCute("DELETE FROM cms_Konsoltitlar WHERE tKonsolID = 0 AND tSparadAv = " & CLng(cCMS_ID))
        ' ##################
      rsDB.Close
      Set rsDB = Nothing
    Con_Close
  ...
  Case "del" ' Radera
      ...
      Con_Open
        ...
        Set rsDB = Server.CreateObject("ADODB.RecordSet")
          ...
          For Each oID IN allID
            SQL = "SELECT * FROM cms_Konsol WHERE kID = " & CLng(oID)
            rsDB.Open SQL, Con, 1, 3
          
            If Not rsDB.Eof Then
              Call AddLogg("KONSOL","RADERA [TOTAL]",rsDB("kID"))

              Set rsBilder = Server.CreateObject("ADODB.RecordSet")
              SQL = "SELECT * FROM cms_Konsoltitlar WHERE tKonsolID = " & CLng(oID)
              rsBilder.Open SQL, Con
                ...
                Do Until rsBilder.EOF
                  ...
                  ImgRemove rsBilder("tBoxart_Konsol")
                  rsBilder.MoveNext
                Loop
              ...
              Con.ExeCute("DELETE FROM cms_Konsoltitlar WHERE tKonsolID = " & CLng(oID))
    ...
    Session.value("PBM_Message")    = "<h2>Information: Radering slutförd</h2><p>De markerade konsoler som du hade behörighet att radera är nu borta.</p><p>Klicka på ""fortsätt"" för att gå vidare...</p>"
    Session.value("PBM_Lank")       = "modul/Databas/Konsol/_show.asp?" & sRebuild
