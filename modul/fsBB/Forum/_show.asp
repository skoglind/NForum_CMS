<% 
  cON_PAGE = "Alla forumkategorier - Forum - CMS"
%>
...
<!--#INCLUDE FILE="../../../_deftop.asp"-->
  ...
  ' #### ALFALIST ####
    ...
    If sAlfa <> Empty Then sAlfaFilter = " WHERE fName LIKE '" & sAlfa & "%' "
  ' ##################
  
  Con_Open
  Set rsDB = Server.CreateObject("ADODB.Recordset")
  SQL = "SELECT *, (SELECT COUNT(*) FROM fsBB_Tradar WHERE tForum = fID) AS fAntalPost FROM fsBB_Forum " & sAlfaFilter & " ORDER BY fNoAllView ASC, fSortNr ASC, fName ASC"
  rsDB.Open SQL, Con
  
  ' #### PAGING ####
  lMaxPosterPerSida = MAXPERPAGE
  lAntalPoster = Con.ExeCute("SELECT COUNT(*) FROM fsBB_Forum " & sAlfaFilter)(0)
  ...
  <div class="datablock rect">
    <div class="legend">Alla forumkategorier</div>
    <div class="editbar">
      ...
      <div style="float: left;">
        ...
        <input type="button" value="Radera" onClick="if(confirm('Vill du radera de markerade posterna?')){doSubmit('datalist','a=del');}" <% If Not GetAcc("CMS333") Then Response.Write(" disabled") %>> |
      ...
      <ul class="list">
        <% For zx = 1 To lMaxPosterPerSida %>
          <% If rsDB.EOF Then Exit For %>
            ...
            <li <% If IsEven Then Response.Write(" class='IO'") %> style="background-image: url('/design/icons/papper_1.png')">  <% If rsDB("fAntalPost") > 0 Then %><span class="number"><strong><% = rsDB("fAntalPost") %></strong><br>poster</span><% End If %> <input type="checkbox" name="chk_id<% = rsDB("fID") %>" value="YES"> <a href="_edit.asp?e=<% = rsDB("fID") %>&<% = sRebuild %>"><% = sEncode(rsDB("fName")) %></a> <span class="status"><% = sEncode(rsDB("fInfo")) %></span> </li>
          <% rsDB.MoveNext %>
        <% Next %>
      </ul>
  ...
  <div class="databox info">
    ...
    <div class="inner">
      <%
      lStatus_0 = Con.ExeCute("SELECT COUNT(*) FROM fsBB_Forum")(0)
      %>
      <table class="list" cellpadding=0 cellspacing=0>
        <tr><td class="td1"> <img src="/design/icons/papper_1_sm.png"> </td><td clasS="td2"> <% = lStatus_0 %> </td><td class="td3"> forumkategorier </td></tr>
      </table>
    </div>
  </div>
